...
public class InitExecutor {
    ...
    private Logger logger = LoggerFactory.getLogger(InitExecutor.class);
            ...
            if (rtn[0].equals("0")) {
                ...
                while (true) {
                    try {
                        ...
                        InstanceDO inst = this.generateInstance(task,taskMap, relaMap.get(task.getTaskId()), triggerTime);
...
//        command = StringUtils.replace(task.getCommand(),"${unix_timestamp}", String.valueOf(triggerTime.getTime() / 1000));
        ...
        for (TaskRelaDO relaDO : relaList) {
            ...
            String preInstanceId = TaskUtils.generateRelaInstanceID(relaDO.getPreId(),
                    taskMap.get(relaDO.getPreId()).getCycle(),
                    triggerTime.getTime(), relaDO.getOffset());
